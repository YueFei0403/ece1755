...
void *compute_txrx_dist( thread_info *curr_thread_info )
{
	...
	__m128 *tx_point_x_simd = (__m128 *) curr_thread_info->tx_point_x;
	__m128 *tx_point_y_simd = (__m128 *) curr_thread_info->tx_point_y;
	__m128 *tx_point_z_simd = (__m128 *) curr_thread_info->tx_point_z;
	...
	for (idx_tx = 0; idx_tx < num_txloop; idx_tx++) {
		...
		float *tx_x = _mm_malloc(4*sizeof(float),16);
		float *tx_y = _mm_malloc(4*sizeof(float),16);
		float *tx_z = _mm_malloc(4*sizeof(float),16);
		float *tx_dist = _mm_malloc(4*sizeof(float),16);
		...
		for (idx_float=0; idx_float<4; idx_float++) {
			...
			for (it_rx = 0; it_rx < TRANS_X * TRANS_Y; it_rx++) {
				...
				float txrx_dist = tx_dist[idx_float] + (float)sqrt(x_comp + y_comp + z_comp);
